      ...
      <form v-on:submit.prevent="findWords">
        <p>
          <label>Find synonyms for
            <input type="text" v-model="phrase" placeholder="word or phrase"> that:</label>
        </p>
        <ul>
          <li>
            <label>sounds like
    ...
    <div class="word-list-container">
      ...
      <ul class="word-list">
        <transition-group name="slideRight" tag="div" appear>
          <li v-for="word in wordList" v-bind:key="word">{{ word }}&nbsp;
  ...
  methods: {
    addWord: function (word) {
      if (this.wordList.indexOf(word) === -1) {
        ...
        this.messages.push({
          type: 'success',
          text: `Added ${word} to WordList.`
        });
      } else {
        this.messages.push({
          type: 'info',
          text: `${word} is already on wordlist.`
        })
      }
    },
